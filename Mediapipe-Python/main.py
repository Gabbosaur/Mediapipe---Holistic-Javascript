...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
	while cap.isOpened():
		...
		try:
			...
			print(degree_sign)
			# Visualize angle
			cv2.putText(image, str(math.trunc(angle_elbow_left)) + '°', 
						tuple(np.multiply(elbow_left, [640, 480]).astype(int)), 
						cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv2.LINE_AA
						)

			cv2.putText(image, str(math.trunc(angle_elbow_right)) + '°', 
						tuple(np.multiply(elbow_right, [640, 480]).astype(int)), 
						cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv2.LINE_AA
						)

			cv2.putText(image, str(math.trunc(angle_shoulder_left)) + degree_sign, 
						tuple(np.multiply(shoulder_left, [640, 480]).astype(int)), 
						cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv2.LINE_AA
						)

			cv2.putText(image, str(math.trunc(angle_shoulder_right)) + '°', 
		...
		mp_drawing.draw_landmarks(image, results.pose_landmarks, mp_pose.POSE_CONNECTIONS,
									...
									mp_drawing.DrawingSpec(color=(0,255,0), thickness=2))	#color=(b,g,r)
